...
var toCamelCase = (str)=>str.replace(/-([a-z])/g, function(g) {
...
function newQueue() {
    const queue = {
        ...
        dequeue: ()=>{
            if (queue.headIdx == queue.tailIdx) throw new Error("Queue is empty");
...
const dataflow = {
    nameToFn: new Map(),
    nodeToUpstreamNodes: new Map(),
    nodeToDownstreamNodes: new Map(),
    updateBatches: newQueue(),
    value: {},
    valueChanged: {},
    inProgress: false,
    errors: [],
    register: function(...fns) {
        const register = (fn, fnName)=>{
            if (!(fn instanceof Function)) throw new Error("Parameter is not a function: " + fn);
            if (dataflow.nameToFn.has(fnName)) throw new Error("Function is already registered: " + fnName);
            ...
            for (const usName of paramNames){
                ...
                if (!dsFns) dataflow.nodeToDownstreamNodes.set(usName, dsFns = []);
                dsFns.push(fnName);
            }
        };
        if (arguments.length == 1 && typeof arguments[0] === "object" && !Array.isArray(arguments[0])) // Accept registration in the form of `dataflow.register({fnName: () => val})
        for (const [fnName1, fn2] of Object.entries(arguments[0]))register(fn2, fnName1);
        else // Accept registration as a list of named functions: `dataflow.register(a, b, c)`
        for (const fn1 of fns)register(fn1, fn1.name);
    },
    set: async function(nameToValuesObj) {
        ...
        const visitNode = (name, visited, visitedInPath, fnVisitor)=>{
            if (visitedInPath.has(name)) throw new Error("Cycle detected, consisting of nodes: " + visitedInPath);
            ...
            if (!visited.has(name)) {
                ...
                if (dsFnNames) for (const dsFnName of dsFnNames){
        ...
        const visitReachableFnsFromParams = (paramNames, fnVisitor)=>{
            ...
            for (const paramName of paramNames)visitNode(paramName, visited, visitedInPath, fnVisitor);
        };
        ...
        const setNodeValue = (name, value, dirtyNodeNamesOut)=>{
            ...
            if (valueChanged) {
                if (DEBUG_DATAFLOW) console.log("Setting:", {
                    [name]: value
                });
            ...
            const dsFnNames = dataflow.nodeToDownstreamNodes.get(name);
            if (dsFnNames) dsFnNames.forEach((dsFnName)=>{
                const dsFn = dataflow.nameToFn.get(dsFnName);
                if (--dsFn.numDirtyDeps == 0) // The current node is the last dependency of the downstream node that
        ...
        if (!dataflow.inProgress) {
            ...
            while(!dataflow.updateBatches.isEmpty()){
                ...
                for (const fn3 of dataflow.nameToFn.values())fn3.numDirtyDeps = 0;
                ...
                for (const [name, value] of Object.entries(updateBatch))setNodeValue(name, value, dirtyNodeNames);
                ...
                while(dirtyNodeNames.size > 0){
                    ...
                    for (const name of dirtyNodeNames){
                        ...
                        for (const paramName of paramNames){
                            if (dataflow.valueChanged[paramName]) someArgChanged = true;
                            const arg = dataflow.value[paramName];
                            args.push(arg);
                            if (DEBUG_DATAFLOW) paramNamesAndArgs[paramName] = arg;
                        }
                        ...
                        if (someArgChanged) {
                            ...
                            if (DEBUG_DATAFLOW) console.log("Calling:", {
                                [name]: paramNamesAndArgs
                            });
                            // Call fn with these params, returning the resulting promise
                            promises.push(fn(...args));
                        } else // Otherwise reuse cached val (we still need to propagate unchanged
                    ...
                    for(var i = 0; i < fnNames.length; i++){
                        ...
                        if (promiseResult.status === "fulfilled") // Cache successful function call results
                        ...
                        } else console.log("Unknown promise result", promiseResult);
                    }
                }
                if (DEBUG_DATAFLOW && !dataflow.updateBatches.isEmpty()) console.log("Starting next dynamic dataflow batch");
            }
            dataflow.inProgress = false;
            if (DEBUG_DATAFLOW) console.log("Dataflow ended");
        }
    },
    connectToDOM: ()=>{
        ...
        let idIdx = 0;
        [
            ...document.querySelectorAll("[from-dataflow]")
        ].forEach((elt)=>{
            ...
            for (const dataflowAttrVal of dataflowAttrValParts){
                const getNodeName = (part)=>{
                    if (!part || !validJSIdent.test(part)) throw new Error("from-dataflow attribute does not specify a valid dataflow node name: " + elt.outerHTML);
                    return part;
                };
                ...
                if (dataflowAttrVal.includes(":class:")) {
                    ...
                    setter = "if (" + nodeName + " !== undefined && " + nodeName + " !== elt.classList.contains('" + className + "')) " + "elt.classList.toggle('" + className + "');";
                    ...
                    setter = (nodeName === "allowBuying_out" ? "console.log('CURR: ', elt.checked);" : "") + "if (elt." + targetAttrName + " !== undefined) " + "elt." + targetAttrName + " = " + definedOrBlank(nodeName) + "; else " + "elt.setAttribute('" + targetAttrName + "', " + definedOrBlank(nodeName) + ");" + (nodeName === "allowBuying_out" ? "console.log('NEW: ', elt.checked);" : ""); // TODO
                } else {
        ...
        const registerListeners = (eventName)=>{
            ...
            [
                ...
                if (elt.tagName.toLowerCase() !== "input") throw new Error("Element with " + attrName + " attribute is not an input element: " + elt.outerHTML);
                const dataflowAttrVal = elt.getAttribute(attrName);
                if (!validJSIdent.test(dataflowAttrVal)) throw new Error(attrName + " attribute does not specify a valid dataflow node name: " + elt.outerHTML);
                elt.addEventListener(eventName, ()=>dataflow.set({
                        [dataflowAttrVal]: getInputValue(elt)
                    }));
        ...
        if (DEBUG_DATAFLOW) console.log("Initial values:", initialValues);
        dataflow.set(initialValues);
    }
};

//# sourceMappingURL=index.0f6d118b.js.map
